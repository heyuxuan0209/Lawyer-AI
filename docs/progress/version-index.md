...
下一版本建议：已进入 v0.7-live-static-prototype。
...
- 新增实时谈案 Copilot 静态原型。
- 使用劳动争议为例，模拟客户/律师分角色实时转写。
- 右侧同步更新案情要点、追问建议、模拟法条卡和方案草稿。
...
- `prototype/app.js`
- `prototype/README.md`
- `docs/02-product/current/live-static-prototype-v0.7.md`

核心结论：

```text
第一版实时原型重点验证咨询过程中的辅助价值，而不是会后文书生成。
法律意见书保留为模拟格式样例，并明确未经律师审核不可发送客户。
```

下一版本建议：

- v0.8-live-feedback：对追问建议、法条卡、方案草稿分别加反馈。
- v0.9-real-transcription：接入上传音频转写或实时 ASR。

## v0.8-polish-for-lawyer-demo
...
- 把实时谈案原型打磨到更适合给律师朋友演示。
- 增强咨询脚本真实性。
- 让右侧辅助信息更像咨询现场的实时 Copilot。
- 让会后产物入口更明显。
- 补充律师演示验证清单。
...
- `prototype/app.js`
- `prototype/styles.css`
- `docs/02-product/current/lawyer-demo-validation-checklist-v0.8.md`

核心结论：

```text
给律师看的重点不是页面，而是实时转写、追问建议、可能相关规则、方案草稿是否能进入咨询现场。
```

下一版本建议：已进入 v0.9-lawyer-feedback-demo。

## v0.9-lawyer-feedback-demo
...
- 在单页实时谈案原型里增加律师现场反馈能力。
- 对追问建议、模拟法条卡、方案草稿分别收集快速反馈。
- 增加右侧实时更新高亮，让律师感知系统随着对话变化。
- 在页面内放入演示验证问题，方便直接拿给律师看。
...
```text
当前版本适合先给律师朋友看，用它验证“实时辅助是否有价值”。
下一步不应继续堆会后文书，而应接入真实音频/转写，验证真实咨询现场能否稳定工作。
```
